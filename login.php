...
if(isset($_POST["submit"])){
	...
	if(mysqli_num_rows($result_user)>0){
		...
		$_SESSION["tuser_id"]=$uid;
		$_SESSION["tuser"]=$_POST["user_name"];
		$_SESSION["tuser_pass"]=$_POST["user_pass"];
		$_SESSION["val_type"]=$val;
			if($val==1){
					header("Location: imagelogin.php");
					die();
				}else if($val==2){
					header("Location: colorlogin.php");
					die();
				}else if($val==3){
					header("Location: gesturelogin.php");
					die();
				}else if($val==4){
					header("Location: blanklogin.php");
					die();
				}
		}else{
